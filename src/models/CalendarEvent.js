...
export default class CalendarEvent {
    ...
    constructor( data, calendar ) {
        ...
        this._data = data;
        this._type = this._checkDataType(data);
    };
    ...
    async init() {
        switch ( this._type ) {
            ...
            case "FullCalendarEvent":
                this._create(this._data, this._type);
                break;
            case "GUID":
                {
                    const doc = await objDatabase.DocumentFromGUID(this._data);
    ...
    async _createWizEventDoc() {
        ...
        await this.addToCalendar(
            doc, newEvent.CALENDAR_START, 
            newEvent.CALENDAR_END, 
            newEvent.CALENDAR_INFO,
            newEvent.CALENDAR_EXTRAINFO
        );
        // change database
        //doc.type = "event";
        //
    ...
    async addToCalendar(doc, strStart, strEnd, strInfo, strExtraInfo) {
        await this._setParamValue(doc, "CALENDAR_START", strStart);
        await this._setParamValue(doc, "CALENDAR_END", strEnd);
        await this._setParamValue(doc, "CALENDAR_INFO", strInfo);
        await this._setParamValue(doc, "CALENDAR_EXTRAINFO", strExtraInfo);
    }

    async removeFromCalendar(doc) {
        await doc.RemoveParam("CALENDAR_START");
        await doc.RemoveParam("CALENDAR_END");
        await doc.RemoveParam("CALENDAR_INFO");
        await doc.RemoveParam("CALENDAR_EXTRAINFO");
    }
